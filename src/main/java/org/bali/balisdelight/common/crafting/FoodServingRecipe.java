...
public class FoodServingRecipe extends CustomRecipe {
        ...
        for (int index = 0; index < container.getContainerSize(); ++index) {
            ...
            if (!selectedStack.isEmpty()) {
                if (cookingPotStack.isEmpty()) {
                    ItemStack mealStack = OvenBlockEntity.getMealFromItem(selectedStack);
                    if (!mealStack.isEmpty()) {
                        cookingPotStack = selectedStack;
                        containerStack = OvenBlockEntity.getContainerFromItem(selectedStack);
                        continue;
                    }
                }
    ...
    public ItemStack assemble(CraftingContainer container, RegistryAccess access) {
        for (int i = 0; i < container.getContainerSize(); ++i) {
            ...
            if (!selectedStack.isEmpty() && selectedStack.is(ModItem.OVEN.get())) {
                ItemStack resultStack = OvenBlockEntity.getMealFromItem(selectedStack).copy();
    ...
    public NonNullList<ItemStack> getRemainingItems(CraftingContainer container) {
        ...
        for (int i = 0; i < remainders.size(); ++i) {
            ...
            } else if (selectedStack.is(ModItem.OVEN.get())) {
                OvenBlockEntity.takeServingFromItem(selectedStack);
